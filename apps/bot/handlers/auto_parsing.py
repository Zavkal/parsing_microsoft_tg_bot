...
from apps.bot.keyboards.auto_parsing_keyboards import auto_parsing_kb, \
...
from apps.parser.service.generate_text_auto_pars import generate_text_auto_pars
...
async def settings_auto_parsing(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback_query.message.edit_text(text="⚙️ Настройка",
...
async def process_time_input(message: types.Message, state: FSMContext):
    ...
    await repo_manager.parser_schedule_repo.update_parser_schedule(parser_name, time_pars=message.text.strip())
...
async def set_monthday(callback: types.CallbackQuery):
    ...
    await repo_manager.parser_schedule_repo.update_parser_schedule(parser_name, day_of_month=day)
    await ParserSchedulerManager().refresh_schedule()
    config = await repo_manager.parser_schedule_repo.get_schedule_by_parser(parser_name=parser_name)
    await callback.message.edit_text(
        text=f"⚙️ Настройки: {parser_name}",
...
async def change_status_auto_pars(callback_query: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await callback_query.message.edit_text(text="Управление парсерами:",
                                           reply_markup=change_status_auto_pars_kb(conf_pars=conf_pars))
...
async def change_pars_status(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await repo_manager.parser_schedule_repo.update_status_pars(
        parser_name=parser_name,
