...
from apps.bot.keyboards.base_menu_keyboards import base_menu_keyboards
...
async def command_start_handler(message: types.Message, state: FSMContext) -> None:
    ...
    new_products, products, sale_products = await get_last_pars(repo_manager=repo_manager)
...
async def back_base_menu_keyboards(callback_query: types.CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    new_products, products, sale_products = await get_last_pars(repo_manager=repo_manager)
